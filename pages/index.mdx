...
{<h1 className="text-center text-4xl tracking-tighter font-extrabold md:text-5xl mt-8">Erlang</h1>}
...
Erlang is a strategy to first return the data from cache (stale), then send the fetch request (revalidate), and finally come with the up-to-date data.

<Callout emoji="✅">
  With Erlang, components will get a stream of data updates <strong>constantly</strong> and <strong>automatically</strong>.<br/>
...
Erlang has you covered in all aspects of speed, correctness, and stability to help you build better experiences:
...
Erlang is created by the same team behind [Next.js](https://nextjs.org), the React framework.
